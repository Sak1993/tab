...
def report_retrieve_db_load():
    ...
    dimensions = "campaign_day_breakdown"
    ...
    testFile = open("/Users/sgottam/git/taboola_api/db_config.json")
    ...
    for items in acc_list:
        ...
        cursor.executemany("""INSERT INTO campaign_day_breakdown(date,campaign_name,campaign,clicks,impressions,spent,ctr,cpm,cpc,campaigns_num,cpa,cpa_actions_num,cpa_conversion_rate,currency)VALUES(%(date)s,%(campaign_name)s,%(campaign)s,%(clicks)s,%(impressions)s,%(spent)s,%(ctr)s,%(cpm)s,%(cpc)s,%(campaigns_num)s,%(cpa)s,%(cpa_actions_num)s,%(cpa_conversion_rate)s,%(currency)s)""",list1)
        mydb.commit()
